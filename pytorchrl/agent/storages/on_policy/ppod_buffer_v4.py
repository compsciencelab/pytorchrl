...
class PPODBuffer(B):
    ...
    demos_data_fields = (prl.OBS, prl.ACT, prl.REW, prl.VAL)

    def __init__(self, size, device, actor, algorithm, envs,
                 frame_stack=1, frame_skip=0, rho=0.1, phi=0.0, gae_lambda=0.95,
        ...
        self.value_error_threshold = np.inf
        self.max_demo_reward = max([d["TotalReward"] for d in self.reward_demos]) if len(self.reward_demos) > 0 else -np.inf
        ...
        self.demos_in_progress = {
            "env{}".format(i + 1): {
                ...
                "Step": 0,
                "DemoLength": -1,
                "ValuePredictionError": 0,
                prl.RHS: None,
            } for i in range(self.num_envs)}

    @classmethod
    def create_factory(cls, size, frame_stack=1, frame_skip=0, rho=0.1, phi=0.0, gae_lambda=0.95,
    ...
    def before_gradients(self):
        ...
        print("\nREWARD DEMOS {}, RHO {}, PHI {}, REWARD THRESHOLD {}, MAX DEMO REWARD {}\n".format(
            len(self.reward_demos), self.rho, self.phi, self.reward_threshold, self.max_demo_reward))
        ...
        for i in range(self.num_envs):
            ...
            if self.demos_in_progress["env{}".format(i + 1)]["Demo"]:
                ...
                self.demos_in_progress["env{}".format(i + 1)]["ValuePredictionError"] += (
                    algo_data[prl.VAL].squeeze() -
                    self.demos_in_progress["env{}".format(i + 1)][prl.RET][demo_step] # step or step + 1?
                ) ** 2
                ...
                if demo_step == self.demos_in_progress["env{}".format(i + 1)]["DemoLength"] - 1:
                    ...
                    for demo in self.reward_demos:
                        ...
                        if self.demos_in_progress["env{}".format(i + 1)]["Demo"]["ID"] == demo["ID"]:
                            import ipdb; ipdb.set_trace()
                            demo["ValuePredictionError"] = self.demos_in_progress["env{}".format(i + 1)]["ValuePredictionError"]
    ...
    def track_potential_demos(self, sample):
        ...
        for i in range(self.num_envs):
            ...
            if sample[prl.DONE2][i] == 1.0:
                ...
                # Compute returns in numpy
                returns = self.potential_demos["env{}".format(i + 1)][prl.REW]
                for step in reversed(range(returns.shape[0] - 1)):
                    returns[step] += returns[step + 1] * self.algo.gamma

                # Add returns to demo
                potential_demo[prl.RET] = returns

                # Compute value error
                error = ((self.potential_demos["env{}".format(i + 1)][prl.RET] -
                         self.potential_demos["env{}".format(i + 1)][prl.VAL]) ** 2).sum()
                ...
                if error >= self.value_error_threshold:
                    ...
                    self.value_error_threshold = min([d["ValuePredictionError"] for d in self.reward_demos])
    ...
    def load_initial_demos(self, initial_human_demos_dir=None, initial_agent_demos_dir=None):
        ...
        for demo_file in initial_human_demos:
            ...
            try:
                ...
                returns = torch.FloatTensor(demo[prl.REW])
                for step in reversed(range(demo[prl.REW].shape[0] - 1)):
                    returns[step] += returns[step + 1] * self.algo.gamma
                new_demo[prl.RET] = returns
        ...
        for demo_file in initial_reward_demos:
            ...
            try:
                ...
                # Uniquely identify demo
                demo_id = str(uuid.uuid4())
                self.initial_agent_demo_ids.append(demo_id)
        ...
        print("\nLOADED {} HUMAN DEMOS {} REWARD DEMOS".format(
            num_loaded_human_demos, num_loaded_reward_demos))

        # Rename initial agent demos dir not to overlap with new agent demos
        os.rename(
            initial_agent_demos_dir,
            os.path.join(os.path.dirname(initial_agent_demos_dir), "initial_agent_demos")
        )
    ...
    def sample_demo(self, env_id):
        ...
        # Sample episode type
        episode_source = np.random.choice(["reward_demo", "env_episode"],
        ...
        if episode_source == "reward_demo" and len(self.reward_demos) > 0:
            ...
            # give priority to higher value error demos
            import ipdb; ipdb.set_trace()
            probs = np.array([p["ValuePredictionError"] for p in self.reward_demos])
    ...
    def save_demos(self):
        ...
        if self.target_agent_demos_dir:
            ...
            for num, demo_pos in enumerate(reward_ranking):
                ...
                # Avoid saving demos already known
                if self.reward_demos[demo_pos]["ID"] in self.initial_agent_demo_ids:
                    continue
