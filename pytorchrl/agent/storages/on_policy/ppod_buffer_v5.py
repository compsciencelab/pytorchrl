...
class PPODBuffer(B):
    ...
    def __init__(self, size, device, actor, algorithm, envs,
        ...
        self.value_error_threshold = 0.0
        self.max_value_error = - np.inf
        ...
        self.demos_in_progress = {
            "env{}".format(i + 1): {
                ...
                "Step": 0,
                "DemoLength": -1,
                "ValuePredictionError": 1000000000,
                prl.RHS: None,
    ...
    def before_gradients(self):
        ...
        print("\nREWARD DEMOS {}, RHO {}, VALUE ERROR THRESHOLD {}, MAX VALUE ERROR {}\n".format(
            len(self.reward_demos), self.rho, self.value_error_threshold, self.max_value_error))
    ...
    def track_potential_demos(self, sample):
        ...
        for i in range(self.num_envs):
            ...
            if sample[prl.DONE2][i] == 1.0:
                ...
                returns = potential_demo[prl.REW]
                ...
                if error >= self.value_error_threshold:
                    ...
                    # Update max value error
                    self.max_value_error = max([d["ValuePredictionError"] for d in self.reward_demos])
    ...
    def sample_demo(self, env_id):
        ...
        # Sample episode type
        episode_source = np.random.choice(["reward_demo", "env_episode"], p=[self.rho, 1.0 - self.rho])
        ...
        if episode_source == "reward_demo" and len(self.reward_demos) > 0:
            ...
            probs = np.array([p["ValuePredictionError"] for p in self.reward_demos]) + 1e-8
    ...
    def check_demo_buffer_capacity(self):
        ...
        if len(self.reward_demos) > self.max_demos:
            for _ in range(len(self.reward_demos) - self.max_demos):
                ...
                # Eject demo with lowest value error
                rewards = np.array([p["ValuePredictionError"] for p in self.reward_demos[self.num_loaded_human_demos:]])
                del self.reward_demos[np.argmin(rewards) + self.num_loaded_human_demos]
